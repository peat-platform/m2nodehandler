...
var parseNetstring = function(ns) {
   ...
   if (payload[length] !== ',') {
      throw new Error('Netstring did not end in ","');
   }
...
var sender = function(config) {
   ...
   dbc.assert(config !== null || config !== undefined);
   ...
   if (config.isMongrel2 || config.asJson) {
      return {
         'send' : function (uuid, connId, status, headers, body) {
            ...
            var message;
            if(!config.asJson)
               message = buildZmqResponse(msg.uuid, msg.connId, msg.status, msg.headers, msg.body);
            else
...
var getHeadersAndMongrel2 = function(data) {
   ...
   console.log(mongrelString);
   console.log(mongrelStringSplit);
   console.log(payload);
