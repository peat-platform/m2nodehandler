...
function sender(params){
   var sender      = zmq.socket(params.type);
   sender.identity = params.id
   ...
   if (params.bind){
      sender.bindSync(params.spec);
      sender.send("prime_q")
   }
   else{
      sender.connect(params.spec);
      sender.send("prime_q")
   }
...
function receiver(params, callback) {

   var receiver      = zmq.socket(params.type)
   receiver.identity = params.id
   ...
   if (params.bind){
      receiver.bindSync(params.spec);
   }
   else{
      receiver.connect(params.spec);
   }
...
function bindToMong2PubQ(params){
   ...
   console.log('setting Publisher ' + process.pid)
...
function bindToInternalPubQ(params){
   console.log('setting Publisher ' + process.pid)

   var pub      = zmq.socket('pub');
   pub.identity = params.id
   pub.bind       (params.spec, function(err) {
      if (err){
         throw err;
      }
...
function bindToSubsQ(params) {
   console.log('setting Subscriber '+process.pid)

   var sub       = zmq.socket('sub')
   sub.identity  = params.id
   sub.connect     (params.spec)
   sub.subscribe   ('')

   return sub.on('message', function (envelope) {
      console.log(sub.identity + ' Sub: ' + envelope + ' \n\r\n\r')
      /*if (envelope != null) {
         var zmqMsg = parse(envelope.toString('utf8'))*/
...
module.exports.bindToMong2PullQ   = bindToMong2PullQ
...
module.exports.bindToSubsQ        = bindToSubsQ
module.exports.receiver           = receiver
module.exports.sender             = sender
module.exports.bindToInternalPubQ = bindToInternalPubQ
